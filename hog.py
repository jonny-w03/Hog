    ...
    def record(score0, score1):
        ...
        if who == 0:
            ...
            if diff > high:
                ...
                print ( str(high) + " point(s)! That's a record gain for Player " + str(who) + "!")
            pre = score1
        return announce_highest(who, pre, high)
    return record
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def avg_dice(rolls = 1, dice = original_function):
        result = 0
        a = 0
        while a<trials_count:
            result += roll_dice(rolls, dice)
            a += 1
        result /= trials_count
        return result
    return avg_dice
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    high = 0
    while count <= 10:
        score = make_averaged(dice, trials_count)(count)
        if score > high:
            high = score
            current = count
        count += 1
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if score+picky_piggy(opponent_score) == opponent_score or picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
